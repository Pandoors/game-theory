//
// Created by Bartosz Kruczek on 02/12/2020.
//
#include<stdio.h>
...
//Sprawdza, czy gracz (człowiek) może wykonać ruch na pole, które sam wybrał. Uniemożliwia oszukiwanie.
...
int main(void) {
    ...
    while (1) {
        ...
        wypisz(plansza);
        printf("\nOto ruch komputera...\n Proszę podać współrzędne swojego ruchu w postaci X_STARTOWE Y_STARTOWE X_NOWE Y_NOWE\n");
        scanf("%d %d %d %d", &x_start, &y_start, &x_nowe, &y_nowe);
        while (1) {
            ...
            if (mozliwy_ruch(plansza, x_start, y_start, x_nowe, y_nowe) == 1) {
                ...
                printf("Pole jest już zajęte, lub ruch niedozwolony. Proszę wybrać inne pole, podając współrzedne postaci X_STARTOWE Y_STARTOWE X_NOWE Y_NOWE\n");
                scanf("%d %d %d %d", &x_start, &y_start, &x_nowe, &y_nowe);
